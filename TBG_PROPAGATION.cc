...
#define BETA      145.0                       					     	// Inverse temperature (1/eV) --> 80 Kelvin
// additional options
...
#define endtime   759.8                                                 // End time (500 fs)
#define timesteps 1e4 													// # of timesteps			
#define fac       10                                                    // Every fac-th value is stored on disc

// Peierls driving
#define w_peierls      0.190                                            // Frequency of Applied Field (in eV)
#define Ax_peierls     0.00643421                                       // Amplitude of Applied Field in x-direction in 1/lconst*AA
#define Ay_peierls     0.00643421                                       // Amplitude of Applied Field in y-direction in 1/lconst*AA
#define Az_peierls     0.0                                              // Amplitude of Applied Field in z-direction in 1/lconst*AA
#define Tpulse         455.927										    // Sigma in 1/eV

// SOURCE DRAIN
#define ESD -0.0002445                                                   // Source drain electric field strength in MV/cm (A->Int(E))
#define TSWITCH 30.4                                                    // Switch on time (20 fs)                        
...
#define BETAB     145.0                                                 // Bath temperature
...
inline double THETA(double t, double tp)
/**
 *  Delta function
 */
{
...
*/
	...
	int t = int(time/h);
    return -Pol*Ax_peierls*pow(sin(PI*time/Tpulse),2.)*sin(w_peierls*time)*(1.-THETA(time, Tpulse))+ASD[t];
}
 ...
 *  -time: Real time coordinate
 */
    return -Ay_peierls*pow(sin(PI*time/Tpulse),2.)*cos(w_peierls*time)*(1.-THETA(time, Tpulse));
}
 ...
 *  -time: real time coordinate
 */
    return Az_peierls*pow(cos(PI*time/Tpulse),2.)*cos(w_peierls*time)*(1.-THETA(time, Tpulse));
}
...
void set_dRHOdt_DOWN(int &dim_new, cvec &TEMP1, cvec &TEMP2, cvec &RHO_t_tk, cvec &dRHO_dt, cvec &Hk_DOWN, dvec &evals_DOWN, vector<cvec*> Hk_DOWN_LIST, double mu)
/**
...
{	
	...
	for(int a=0; a<dim_new; a++)
	{
		for(int b=0; b<dim_new; b++)
		{
			...
			//if(TEMP2[fq(a,b,dim_new)].real()>1e-9) cout << TEMP2[fq(a,b,dim_new)] << " ";
		}
	}		
	...
	{
		dRHO_dt[i] += TEMP2[i];
		//if(TEMP2[i].real()>1e-8) cout << TEMP2[i] << " ";
	}	
#endif 		
}
...
void Hk_bands_DOWN(vector<dvec> &K_PATH, dvec &ASD, double &Pol, const string& filename, int &numprocs, int &myrank)
/**
 ...
 */
{
	...
	vector<int> dimensions(num_kpoints_path);
	ifstream input;
	
	// Read in dimension of matrices
	input.open("../DIMENSIONS_PATH.dat");
	...
	// For fix numer of bands!!
	dim_new = dimensions[0];
	...
	for(int k=myrank; k<num_kpoints_path; k+=numprocs)
	{
		...
		ReadInMAT(dim_new, Hk_DOWN_LIST, "../HK_DOWN_LIST/HK_DOWN_LIST_PATH_"+to_string(k)+".dat");
		set_Hk_DOWN(dim_new, Hk_DOWN, Hk_DOWN_LIST, ASD, Pol, starttime);
 ...
 */
{
	...
	dvec Nv_t(timesteps, 0.);
	dvec Nc_t(timesteps, 0.);
	...
	{   
		...
		for(int i=0; i<dim_new; ++i)			
		{
			for(int j=0; j<dim_new; j++){
				(*RHO_t[0])[fq(i, j, dim_new)] = delta(i,j)*fermi(real((*Hk_DOWN_LIST[0])[fq(i, j, dim_new)]), mu+delta_mu);// + 0.01*(delta(i,j)-1.) +0.05*delta(i,j)*delta(i,1) -0.05*delta(i,j)*delta(i,0); 
		...
		set_dHkdAy_DOWN(dim_new, Hk_DOWN[0], Hk_DOWN_LIST, ASD, Pol, 0.0);
		...
		for(int i=0; i<dim_new; ++i)
		{
			...
			if(i<dim_new/2)
			{
				Nv_t[0]	+= 	real((*RHO_t[0])[fq(i,i,dim_new)])/double(num_kpoints_BZ_full);
			}	
			else
			{
				Nc_t[0]	+= 	real((*RHO_t[0])[fq(i,i,dim_new)])/double(num_kpoints_BZ_full);
			}				
		}
		for(int t=0; t<timesteps-1; t++)
		{
			...
			if(t==0)
			{
				...
				set_dRHOdt_DOWN(dim_new, TEMP1[0], TEMP2[0], RHO_t[0][0], dRHO_dt[0][0], Hk_DOWN[0], evals_DOWN[0], Hk_DOWN_LIST, mu+delta_mu);
#ifndef NO_OMP    	
	#pragma omp parallel for                                 
#endif						
				for(int i=0; i<dim_new*dim_new; ++i)
				...
				// Calculate adiabatic 'groundstate' density matrix and trasnformation matrix
				diagonalize_DOWN(Hk_DOWN[0], evals_DOWN[0], dim_new);
				for(int i=0; i<dim_new; ++i)			
				{
					for(int j=0; j<dim_new; j++){
						(*TEMP6)[fq(i, j, dim_new)] = delta(i,j)*fermi((*evals_DOWN)[i], mu+delta_mu);
						(*SMAT_0)[fq(i, j, dim_new)] = (*Hk_DOWN)[fq(i, j, dim_new)];
					}		
				}
				// Calculation of current x
				set_dHkdAx_DOWN(dim_new, Hk_DOWN[0], Hk_DOWN_LIST, ASD, Pol, h*double(t+1));
				times(RHO_t[1][0], Hk_DOWN[0], TEMP2[0]);
				
				// Transform current operator to instant. band basis
				times_nd(Hk_DOWN[0], SMAT_0[0], TEMP4[0]);
				times(SMAT_0[0], TEMP4[0], Hk_DOWN[0]);	
				
				// Calulation of adiabatic groundstate current
				times(TEMP6[0], Hk_DOWN[0], TEMP4[0]);
				
				// Calculation of current y
				set_dHkdAy_DOWN(dim_new, Hk_DOWN[0], Hk_DOWN_LIST, ASD, Pol, h*double(t+1));
				times(RHO_t[1][0], Hk_DOWN[0], TEMP3[0]);
				
				// Transform current operator to instant. band basis
				times_nd(Hk_DOWN[0], SMAT_0[0], TEMP5[0]);
				times(SMAT_0[0], TEMP5[0], Hk_DOWN[0]);	
				
				// Calulation of adiabatic groundstate current
				times(TEMP6[0], Hk_DOWN[0], TEMP5[0]);
				for(int i=0; i<dim_new; ++i)
				{
					...
					if(i<dim_new/2)
					{
						Nv_t[t+1] += real((*RHO_t[1])[fq(i,i,dim_new)])/double(num_kpoints_BZ_full);
					}	
					else
					{
						Nc_t[t+1] += real((*RHO_t[1])[fq(i,i,dim_new)])/double(num_kpoints_BZ_full);
					}					
				}	
				Nv_t[t+1] = real((*RHO_t[1])[fq(0,0,dim_new)])/double(num_kpoints_BZ_full);
				Nc_t[t+1] = real((*RHO_t[1])[fq(1,1,dim_new)])/double(num_kpoints_BZ_full);			
			}
			// Two step Adams–Bashforth method
			else
			{	// 2-step Adams predictor	
				set_Hk_DOWN(dim_new, Hk_DOWN[0], Hk_DOWN_LIST, ASD, Pol, h*double(t));
				set_dRHOdt_DOWN(dim_new, TEMP1[0], TEMP2[0], RHO_t[1][0], dRHO_dt[1][0], Hk_DOWN[0], evals_DOWN[0], Hk_DOWN_LIST, mu+delta_mu);
#ifndef NO_OMP    	
	#pragma omp parallel for                                 
#endif								
				for(int i=0; i<dim_new*dim_new; ++i)
				...
				set_Hk_DOWN(dim_new, Hk_DOWN[0], Hk_DOWN_LIST, ASD, Pol, h*double(t+1));
				set_dRHOdt_DOWN(dim_new, TEMP1[0], TEMP2[0], RHO_t[2][0], dRHO_dt[2][0], Hk_DOWN[0], evals_DOWN[0], Hk_DOWN_LIST, mu+delta_mu);
#ifndef NO_OMP    	
	#pragma omp parallel for                                 
#endif							
				for(int i=0; i<dim_new*dim_new; ++i)
				...
				for(int i=0; i<dim_new; ++i)
				{
					...
					if(i<dim_new/2)
					{
						Nv_t[t+1] += real((*RHO_t[2])[fq(i,i,dim_new)])/double(num_kpoints_BZ_full);
					}	
					else
					{
						Nc_t[t+1] += real((*RHO_t[2])[fq(i,i,dim_new)])/double(num_kpoints_BZ_full);
					}						
				}
				temp0 = RHO_t[0];
				temp1 = RHO_t[1];
	...
	MPI_Allreduce(MPI_IN_PLACE, &Nv_t[0], timesteps, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(MPI_IN_PLACE, &Nc_t[0], timesteps, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	...
	if(myrank==0)
	{
		ofstream myfile ("DATA/E_t"+POL+to_string(num)+".dat");
		if (myfile.is_open())
		{
	...
	if(myrank==0)
	{
		ofstream myfile ("DATA/n_t"+POL+to_string(num)+".dat");
		if (myfile.is_open())
		{
			for(int t=0; t<timesteps; t+=fac)
			{   
	...
	if(myrank==0)
	{
		ofstream myfile ("DATA/nv_t"+POL+to_string(num)+".dat");
		if (myfile.is_open())
		{
			for(int t=0; t<timesteps; t+=fac)
			{   
	...
	if(myrank==0)
	{
		ofstream myfile ("DATA/nc_t"+POL+to_string(num)+".dat");
		if (myfile.is_open())
		{
			for(int t=0; t<timesteps; t+=fac)
			{   
	...
	if(myrank==0)
	{
		ofstream myfile ("DATA/C_t"+POL+to_string(num)+".dat");
		if (myfile.is_open())
		{
			for(int t=0; t<timesteps; t+=fac)
			{   
...
#pragma omp parallel 
	...
	if(myrank==0)
	{
		ofstream myfile ("DATA/ASD_t.dat");
		if (myfile.is_open())
		{
			for(int tt=0; tt<timesteps; tt+=fac)
			{
	...
	if(myrank==0)
	{
		ofstream myfile ("DATA/A_t.dat");
		if (myfile.is_open())
		{
			for(int tt=0; tt<timesteps; tt+=fac)
			{
				tts = double(tt)*h;
				myfile << Ax_t(LEFT, tts, ASD) << " " << Ay_t(tts) << " " << Az_t(tts) << endl;
			}	
			myfile.close();
		}
		else cout << "Unable to open file" << endl;	
	...
	// Calculate bands as conistency check
	//Hk_bands_DOWN(K_PATH, ASD, LEFT, "bands_DOWN.dat", numprocs, myrank);
	
	for(int i=1; i<2; i++)
	{
		AB2_propatation_DOWN(mu, delta_mu, BZ_PATCH, ASD, LEFT, i, numprocs, myrank);
		AB2_propatation_DOWN(mu, delta_mu, BZ_PATCH, ASD, RIGHT, i, numprocs, myrank);
