...
#define BETA      145.0                       					     	// inverse temperature (1/eV)
// additional options
...
#define lim 2                                                           // window for energies: [-w_peierls*window,w_peierls*window)
...
{	
	...
	limits[0] = NATOM/2-lim;
	limits[1] = NATOM/2+lim;
	dim_new = limits[1]-limits[0];
...
void Calc_List_PATH(double &mu, dvec &evals, cvec &Hk, vector<dvec> &BZ_FULL, const dvec &lvec, vector<dvec> &UNIT_CELL, vector<cvec*> Hk_Taylor, int &numprocs, int &myrank)
/**
 * Calculate bands of Hk(k) for high symmetry path and store them in "bands.txt":
 *  -mu: Chemical potential
 *  -evals: Real vector[NATOM] to store eigenvalues
 ...
 */
{
	...
	if(myrank==0)
	{
		myfile.open("DIMENSIONS_PATH.dat");
		if (myfile.is_open())
		{
			for(int k=0; k<num_kpoints_BZ_full; k++)
			{
...
//	Read in vector of k-points
	...
	//Calc_List_PATH(mu, evals, Hk[0], K_PATH, lvec, UNIT_CELL, Hk_Taylor, numprocs, myrank);
	Calc_List(mu, evals, Hk[0], BZ_PATCH, lvec, UNIT_CELL, Hk_Taylor, numprocs, myrank);
